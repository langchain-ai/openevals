...
ls.describe("json", () => {
    ...
    ls.test(
        ...
        async ({ inputs, referenceOutputs }) => {
          ...
          const result = await evaluator({ outputs, referenceOutputs: referenceOutputs.referenceOutputs });
          expect(result).toBeDefined();
          expect(result.length).toBe(2);
          expect(result[0].key).toBe("a");
          expect(result[0].score).toBe(1.0);
    ...
    ls.test(
        ...
        async ({ inputs, referenceOutputs }) => {
          ...
          let result = await evaluator({ outputs, referenceOutputs: referenceOutputs.referenceOutputs });
