...
def test_pyright_extraction_strategy_default(outputs, expected_result):
...
@pytest.mark.langsmith(output_keys=["outputs"])
@pytest.mark.parametrize("outputs, expected_result", CODE_EXAMPLES)
def test_mypy_extraction_strategy_default(outputs, expected_result):
    mypy_evaluator = create_mypy_evaluator()
